...
from reportlab.lib.colors import grey, whitesmoke, HexColor
...
class ReportService:
    ...
    def generate_training_report(
        ...
        for session in sessions:
            ...
            attendance_result = self.db_session.execute(attendance_query)
            ...
            coach_result = self.db_session.execute(coach_query)
            ...
            feedback_result = self.db_session.execute(feedback_query)
    ...
    async def generate_attendance_report(
        ...
        session_result = self.db_session.execute(session_query)
    ...
    async def _generate_athlete_attendance_report(
        ...
        athlete_result = self.db_session.execute(athlete_query)
    ...
    async def generate_training_report_pdf(
        ...
        report_data = self.generate_training_report(
            start_date=start_date,
            end_date=end_date,
            user_id=user_id,
            user_role=user_role
    ...
    async def generate_monthly_report_pdf(
        ...
        try:
            ...
            training_report = self.generate_training_report(start_date, end_date, user_id, user_role)
        ...
        try:
            ...
            if training_report.data:
                ...
                for cell in table.rows[0].cells:
                    cell.fill.solid()
                    cell.fill.fore_color.rgb = HexColor(79, 129, 189)
                    cell.text_frame.paragraphs[0].font.color.rgb = HexColor(255, 255, 255)
            ...
            for week_num in sorted(weeks.keys())[:4]:  # Limit to first 4 weeks
                ...
                for idx, session in enumerate(sorted_sessions[:rows-1]):
                    ...
                    if session['is_independent']:
                        ...
                        table.cell(idx+1, 1).fill.fore_color.rgb = HexColor(242, 220, 179)  # Light orange
